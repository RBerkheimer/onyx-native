...
(defn create-native
    ([task-name batch-size batch-timeout
    ...
    ([task-name batch-size batch-timeout
      fqclassname ctr-class ctr-args
      lib-name init-args]
    (let [jentry (cat/create-method task-name batch-size batch-timeout fqclassname ctr-class ctr-args)]
    (-> jentry
    (assoc :onyx/fn :onyx-native.instance.bind/method)
    (assoc :native-instance/lib-name lib-name)
    (assoc :native-instance/init-args init-args)
